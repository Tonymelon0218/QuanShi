...
u8 Mod_DpRx_SourceDetection(void)
{
    ...
    {
        return TRUE;
        // return Drv_DpRx_AuxDcDet();  // Default检测AUX
    }
...
#if 0 // 25110301: g_stHdmiEdidInfo 结构体被屏蔽了，编译不通过。屏蔽下面代码强制执行else
...
#endif
#if DPRX_DSC_DEC_SUPPORT == ENABLED
        if ((g_stDpRx.ucDscFifoEmpty | g_stDpRx.ucDscFifoFull) == 0x00) // 251105: 原先是(g_stDpRx.ucDscFifoEmpty | g_stDpRx.ucDscFifoEmpty)
        {
